...
# Per-term collection
# ---------------------------------------------------------------------------
def collect_term(term, size, max_pages, delay, max_workers=8):
    print(f"\n  Search term: '{term}'")
    ...
    if total_pages > 1:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(fetch_page, term, p, size, session, delay): p
...
# Enrichment 
# ---------------------------------------------------------------------------
def enrich_single_project(row, delay):
    session = requests.Session()
...
def main():
    ...
    for term in terms:
        try:
            raw_projects = collect_term(term, size, args.max_pages, args.delay, args.page_workers)
    ...
    kept_df = pd.DataFrame(kept_rows).drop_duplicates(subset="project_id").reset_index(drop=True)
    print(f"\nDeduplicated kept projects: {len(kept_df)}")

    # Parallel enrichment
    if enrich and len(kept_df) > 0:
        print(f"\nEnriching {len(kept_df)} projects in parallel...")
        enriched_rows = []
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = {
                executor.submit(enrich_single_project, row.to_dict(), args.delay): idx
